...
try: # Try to create the table
    conn.execute('''Drop table DataTweetApi_users''') # Drop the Users table
    conn.commit() # Commit the changes
    print('DataTweetApi_users table dropped.') # Print Users table dropped.
except: # If there is an error
    print('DataTweetApi_users table did not exist') # Print Users table did not exist

# create Users table in database
cur.execute('''CREATE TABLE DataTweetApi_users(Users_ID INTEGER PRIMARY KEY NOT NULL, Username TEXT NOT NULL, Email TEXT NOT NULL,
RoleLvl INTEGER NOT NULL, About TEXT NOT NULL, Password TEXT NOT NULL); ''')
conn.commit() # Commit the changes
print('DataTweetApi_users table created.') # Print Users Table created.

#key
try:
    conn.execute("PRAGMA key='BLhgpCL81fdLBk23HkZp8BgbT913cqt0';  -- correct")
except:
    print('PRAGMA key didnt executed')

#data to be inserted into table
#Insert David Markel into the table
nm = str(Encryption.cipher.encrypt(b'David Markel').decode("utf-8")) # Encrypt Username
...
cur.execute("Insert Into DataTweetApi_users ('Username','Email','RoleLvl','About','Password') "
            "Values (?, ?, ?,?,?)",(nm, ph, 3, "expertise is in full stack development will work "
...
nm = str(Encryption.cipher.encrypt(b'Dylan Gire').decode("utf-8")) # Encrypt Username
...
cur.execute("Insert Into DataTweetApi_users ('Username','Email','RoleLvl','About','Password') "
            "Values (?, ?, ?,?,?)", (nm, ph, 2,"Don’t necessarily have an expertise, but experience in Python, "
...
nm = str(Encryption.cipher.encrypt(b'Abraham Beltran').decode("utf-8")) # Encrypt Username
...
nm = str(Encryption.cipher.encrypt(b'Tommy Chong').decode("utf-8")) # Encrypt Username
...
cur.execute("Insert Into DataTweetApi_users ('Username','Email','RoleLvl','About','Password') "
            "Values (?, ?, ?,?,?)", (nm, ph, 2,"Experience working in full stack development. "
...
nm = str(Encryption.cipher.encrypt(b'Victor Marques').decode("utf-8")) # Encrypt Username
...
for row in cur.execute('SELECT * FROM DataTweetApi_users;'): # For each row in the table
